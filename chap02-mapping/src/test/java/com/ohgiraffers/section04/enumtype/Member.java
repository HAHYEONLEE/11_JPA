...
public class Member {
    ...
    //@Enumerated(EnumType.ORDINAL) // enum을 상수 값 사용한다는 의미 - 기본 값(기본이라 생략 가능)
                                  // 장: 데이터 크기가 작다. 단: 이미 저장되어있는 상수 값을 변경할 수 없다.(중간에 데이터 넣으면 꼬임)
    @Enumerated(EnumType.STRING)  // enum을 문자 값 사용한다는 의미(장,단점은 상수 값의 반대)
    private RoleType memberRole;  // 0,1 의 숫자값, ADMIN, USER 값 중 하나를 가질 수 있다.
    @Column(name="status")
